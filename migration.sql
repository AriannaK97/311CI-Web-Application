...
update import._311_service_requests_street_lights_one_out set incident_id=uuid_in(md5(random()::text || clock_timestamp()::text)::cstring);
...
select tmp.incident_id,
       ...
       tmp.status::VARCHAR, tmp.d_id::uuid from
    (SELECT v.incident_id,
            case when coalesce(creation_date, 'NULL') = '' then NULL when creation_date = E'\r' then NULL else coalesce(creation_date, 'NULL')  end creation_date,
...
INSERT INTO alley_lights_out (ID) SELECT incident_id FROM import._311_service_requests_street_lights_one_out;
...
update import.debris set incident_id=uuid_in(md5(random()::text || clock_timestamp()::text)::cstring);
...
select tmp.incident_id::uuid, tmp.current_activity::VARCHAR ,tmp.if_yes_where_is_the_debris_located::VARCHAR,
